...
if [ -z "$1" ]; then
    ...
    TEST_DIR=$(ls -td "${HARNESS_DIR}"/results/taskmaster-* 2>/dev/null | head -1)
    if [ -z "$TEST_DIR" ]; then
...
# Archive if passed
if [ "$FAIL_COUNT" -eq 0 ]; then
    # Determine harness directory from test directory path
    HARNESS_DIR="$(dirname "$(dirname "$TEST_DIR")")"
    ARCHIVE_DIR="${HARNESS_DIR}/archive/$(basename $TEST_DIR)"
    mkdir -p "$ARCHIVE_DIR"
